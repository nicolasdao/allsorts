...
pub fn debug_mapping(mapping: &HashMap<u16, u16>, font_name: &str) -> String {
...
pub fn diagnose_subset_issues(
    ...
    if subset_font.len() > 0 {
        ...
        if mapping.len() < 10 {
            report
                .encoding_issues
                .push("Font has very few glyphs".to_string());
        }
    }
    ...
    if !report.broken_composites.is_empty() {
        report
            .recommendations
            .push("Run update_composite_references to fix broken composite glyphs".to_string());
    }

    if !report.encoding_issues.is_empty() {
        report
            .recommendations
            .push("Consider using CmapTarget::Unicode for web compatibility".to_string());
    }
