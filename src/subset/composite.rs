...
pub fn update_composite_references(
    ...
    let (glyf_offset, loca_offsets) = {
        ...
        let glyf_record = offset_table
            .table_records
            .iter()
            .find(|record| record.table_tag == tag::GLYF);
        let loca_record = offset_table
            .table_records
            .iter()
            .find(|record| record.table_tag == tag::LOCA);
        let head_record = offset_table
            .table_records
            .iter()
            ...
            (glyf_record, loca_record, head_record)
        {
            ...
            let head_data = sfnt
                .scope
                .offset_length(head_record.offset as usize, head_record.length as usize)?;
            let head = head_data.read::<HeadTable>()?;

            // Parse maxp table to get number of glyphs
            let maxp_record = offset_table
                .table_records
                .iter()
            ...
            let maxp_data = sfnt
                .scope
                .offset_length(maxp_record.offset as usize, maxp_record.length as usize)?;
            let maxp = maxp_data.read::<MaxpTable>()?;

            // Parse loca table to get glyph offsets
            let loca_data = sfnt
                .scope
                .offset_length(loca_record.offset as usize, loca_record.length as usize)?;
            let loca =
                loca_data.read_dep::<LocaTable<'_>>((maxp.num_glyphs, head.index_to_loc_format))?;
        ...
        } else if offset_table
            .table_records
            .iter()
            .any(|r| r.table_tag == tag::CFF)
        {
...
fn update_ttf_composites(
    ...
    for glyph_id in 0..loca_offsets.len().saturating_sub(1) {
        ...
        if end > start {
            ...
            let num_contours =
                i16::from_be_bytes([font_data[glyph_offset], font_data[glyph_offset + 1]]);
