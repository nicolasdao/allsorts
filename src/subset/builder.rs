...
impl<'a> SubsetBuilder<'a> {
    ...
    pub fn with_characters(mut self, chars: &str) -> Result<Self, SubsetError> {
        ...
        let provider_clone = Box::new(CloneableProvider { provider: self.provider });
        let mut font = Font::new(provider_clone)
    ...
    pub fn build(self) -> Result<SubsetResult, SubsetError> {
        ...
        let mut result = if let Some(ref pdf_ctx) = self.pdf_context {
            ...
            use crate::subset::result::{FontInfo, FontFormat, SubsetStats};
            
            let provider_clone = &CloneableProvider { provider: self.provider };
            ...
            // Get font info
            let original_size = self.provider.read_table_data(tag::HEAD)
                .map(|data| data.len())
                .unwrap_or(0);
            ...
            SubsetResult {
                ...
                reverse_mapping: pdf_result.glyph_mapping.iter()
                    .map(|(&k, &v)| (v, k))
                    .collect(),
                ...
                original_info: FontInfo {
                    ...
                    max_gid: get_glyph_count(self.provider).unwrap_or(0).saturating_sub(1),
                    size: original_size,
