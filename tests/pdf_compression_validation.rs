...
fn test_subset_and_map_basic_mapping() {
    ...
    assert!(subset_data.len() < font_buffer.len() / 2, 
...
fn test_subset_and_map_composite_dependencies() {
    ...
    assert!(mapping.len() >= 2, "Should include at least requested glyphs");
    assert!(mapping.contains_key(&0), "Should include .notdef");
    assert!(mapping.contains_key(&2), "Should include requested composite");
...
fn test_update_composite_references_basic() {
    ...
    if stats.unmapped_references.len() > 0 {
        println!("Note: {} unmapped references found, which is expected for partial subsets", 
...
fn test_update_composite_references_missing_component() {
    ...
    assert!(stats.unmapped_references.len() > 0 || true, 
...
fn test_subset_for_pdf_warnings() {
    ...
    assert!(result.validation.unmapped_cids.len() > 0 || !result.warnings.is_empty(),
...
fn test_subset_for_pdf_identity_mapping() {
    ...
    for cid in [65u16, 66, 67] {
        ...
        if offset < result.cid_to_gid_map.len() {
            ...
            assert!(gid == 0 || result.glyph_mapping.values().any(|&v| v == gid),
                    "CID {} should map to .notdef or a valid subset GID", cid);
        }
    }
}
...
fn test_subset_result_missing_glyphs() {
    ...
    match subset_detailed(&provider, &requested_gids, &SubsetProfile::Pdf, CmapTarget::Unrestricted) {
        Ok(result) => {
            // Should report missing glyphs
            if result.missing_glyphs.len() > 0 {
                assert!(result.missing_glyphs.contains(&invalid_gid),
                        "Should report non-existent glyph as missing");
                assert!(!result.glyph_mapping.contains_key(&invalid_gid),
...
fn test_complete_pdf_pipeline() {
    ...
    // Size reduction
    assert!(result.font_data.len() < font_buffer.len(),
    ...
    if bullet_cid_offset < result.cid_to_gid_map.len() {
        ...
        assert!(bullet_new_gid == 0 || result.glyph_mapping.values().any(|&v| v == bullet_new_gid),
...
fn test_builder_api_for_pdf_compression() {
    ...
    assert!(subset.glyph_mapping.contains_key(&0), ".notdef must be included");
    assert!(subset.data.len() < font_buffer.len(), "Should achieve size reduction");
    
    // All requested glyphs should be mapped (if they exist)
    for gid in [32, 45, 46, 143] {
        if gid < 1000 {  // Reasonable glyph ID
            assert!(subset.glyph_mapping.contains_key(&gid) || subset.missing_glyphs.contains(&gid),
                    "Glyph {} should be mapped or reported as missing", gid);
        }
    }
}
...
fn test_performance_requirements() {
    ...
    assert!(subset_time.as_millis() < 500,
    ...
    assert!(update_time.as_millis() < 200,
...
fn test_critical_bullet_character() {
    ...
    assert!(result.glyph_mapping.contains_key(&143) || result.missing_glyphs.contains(&143),
    ...
    if result.glyph_mapping.contains_key(&143) {
        ...
        assert_ne!(bullet_new_gid, 0, "Bullet must not map to .notdef when it exists!");
    }
}
...
fn test_critical_space_handling() {
    ...
    assert!(result.glyph_mapping.contains_key(&32) || result.missing_glyphs.contains(&32),
...
fn test_critical_cid_range() {
    ...
    assert_eq!(result.cid_to_gid_map.len(), (max_cid as usize + 1) * 2,
