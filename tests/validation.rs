...
use allsorts::subset::validation::{
    debug_mapping, diagnose_subset_issues, validate_mapping_coverage,
};
...
fn test_validate_mapping_coverage_complete() {
    ...
    let result = validate_mapping_coverage(&mapping, &[0, 1, 2], None);
...
fn test_validate_mapping_with_cid_map() {
    ...
    let result = validate_mapping_coverage(&mapping, &[0, 1, 2], Some(&cid_to_gid));
...
fn test_debug_mapping_output() {
    let mapping = HashMap::from([(0, 0), (100, 1), (200, 2)]);
